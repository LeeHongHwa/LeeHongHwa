...
- (IBAction)clickDismissButton:(UIButton *)sender
{
    ...
    //패스워드 = RE 패스워드 && 입력된값이 있을경우 && 이미등록된 아이디가 있을경우
    if ([PW isEqualToString:rePW] && ![ID isEqualToString:@""] && ![PW isEqualToString:@""] && [[DataCenter sharedInstance] getUserIDwithcurrentID:ID] == nil)
    {
    ...
    //아이디 중복
    } else if([[DataCenter sharedInstance] getUserIDwithcurrentID:ID] != nil)
    {
        UIAlertController *IdOrPwOverlap = [UIAlertController alertControllerWithTitle:@"아이디 중복"
                                                                             message:@"중복된 아이디입니다."
                                                                      preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"확인"
                                                           style:UIAlertActionStyleDefault
                                                         handler:nil];
        [IdOrPwOverlap addAction:okAction];
        [self presentViewController:IdOrPwOverlap animated:YES completion:nil];
    }
    else
    {   //패스워드 불일치
